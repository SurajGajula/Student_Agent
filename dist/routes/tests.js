...
import { getGeminiClient, isVertexAI, getProjectId, getAccessToken, getAuthClient, initializeGeminiClient } from '../services/gemini.js';
import { authenticateUser } from './middleware/auth.js';
import { checkTokenLimit, recordTokenUsage } from '../services/usageTracking.js';
const router = express.Router();
// Generate test questions using Gemini
async function generateTestQuestions(noteContent) {
...
Return ONLY the JSON object, no additional text or markdown formatting.`;
    ...
    if (!isVertexAI() || !getAuthClient() || !getProjectId()) {
        throw new Error('Gemini is not configured for Vertex AI. Ensure studentagent.json exists and the service account has the "Vertex AI User" role.');
    }
...
router.post('/generate', authenticateUser, async (req, res) => {
    ...
    try {
        ...
        const { noteId, noteName, noteContent } = req.body;
        if (!noteId || !noteName) {
