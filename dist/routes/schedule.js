...
import { getGeminiClient, isVertexAI, getProjectId, getAccessToken, getServiceAccountKey, getAuthClient, initializeGeminiClient } from '../services/gemini.js';
import { authenticateUser } from './middleware/auth.js';
import { checkTokenLimit, recordTokenUsage } from '../services/usageTracking.js';
const router = express.Router();
const upload = multer({ storage: multer.memoryStorage() });
router.post('/parse-schedule', authenticateUser, upload.single('image'), async (req, res) => {
...
- Return ONLY JSON array, no other text`;
        ...
        if (!isVertexAI() || !getAuthClient() || !getProjectId()) {
            ...
            throw new Error(`Gemini is not configured for Vertex AI. Ensure studentagent.json exists and the service account ` +
        ...
        if (!response.ok) {
            ...
            if (response.status === 403 && errorData.error?.status === 'PERMISSION_DENIED') {
                const serviceAccountEmail = getServiceAccountKey()?.client_email || 'your service account';
                throw new Error(`Service account lacks Vertex AI permissions. ` +
        ...
        try {
            ...
            classes = JSON.parse(cleanedResponse.trim());
        }
        catch (parseError) {
